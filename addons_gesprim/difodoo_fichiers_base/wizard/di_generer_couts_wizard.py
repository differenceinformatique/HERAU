...
class DiGenCoutsWiz(models.TransientModel):
    ...
    def di_generer_cmp(self, di_product_id, di_date, premier_mouv_date=False, recursion=0):
            ...
            if premier_mouv_date:
                ...
                if not cout_veille and ((self.di_cde_ach and (date_veille >= premier_mouv_date.date()))or(not self.di_cde_ach and (date_veille >= premier_mouv_date.date()))) :
                    if recursion > 500 :
                        ...
                        (qte, mont, nbcol, nbpal, nbpiece, poids, dernier_id, nouveau_cmp) = self.env['stock.move'].di_somme_quantites_montants(di_product_id, date_cr_cout_veille, date_veille, self.di_cde_ach, dernier_id_cout_veille)
                        if qte == 0:
                            data = {
                            'di_date': date_veille,
                        ...
                        self.di_generer_cmp(di_product_id, date_veille, premier_mouv_date, recursion)
                        cout_veille = self.env['di.cout'].search(['&', ('di_product_id', '=', di_product_id.id), ('di_date', '=', date_veille)], limit=1)
